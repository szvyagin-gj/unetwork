...
enum WSOpcodes { CONTINUATION = 0, TEXT = 0x1, BINARY = 0x2, CLOSE = 0x8, PING = 0x9, PONG = 0xA };
...
static constexpr unsigned int max_frame_header_size = sizeof(WSHeader) + sizeof(uint64_t);
...
To union_conv_ptr(From ptr) {
  ...
  // Yes I know that reading field which had not been written is UB. but what to do?
  return ptrUn.t;
}
...
std::vector<std::byte> frames::DataFrame(std::span<const std::byte> data, bool is_text,
...
static void make_control_frame(WSOpcodes opcode, std::vector<std::byte>& frame) {
  frame.resize(sizeof(WSHeader));
  WSHeader* hdr = union_conv_ptr<WSHeader*>(frame.data());
  hdr->bytes = 0;
  hdr->bits.fin = 1;
...
CloseStatusInt ReadWSFrame(FrameParserState& frame, userver::engine::io::ReadableBase* io, unsigned max_payload_size)
{
  ...
  if (payloadLen > 0) {
    ...
    if (mask.mask32) xor_mask_inplace((uint8_t*)frame.payload.data(), frame.payload.size(), mask);
  }
  ...
  switch (opcode) {
    ...
    case CLOSE:
      ...
      if (frame.payload.size() >= 2)
        frame.remoteCloseStatus =
            be16toh(*(union_conv_ptr<CloseStatusInt const*>(frame.payload.data())));
      break;
    case TEXT:
      frame.isText = true;
      [[fallthrough]];
